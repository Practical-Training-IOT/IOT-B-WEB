  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt"width="180">
            ...
            <el-table-column align="left" label="告警名称" prop="name" width="120" />

            <el-table-column align="left" label="设备ID" prop="deviceId" width="120" />

            <el-table-column align="left" label="告警级别" prop="alertLevel" width="120">
    <template #default="scope">
    {{ filterDict(scope.row.alertLevel,alert_levelOptions) }}
    </template>
</el-table-column>
            <el-table-column align="left" label="告警类型" prop="alertType" width="120">
    <template #default="scope">
    {{ filterDict(scope.row.alertType,alert_typeOptions) }}
    </template>
</el-table-column>
            <el-table-column align="left" label="告警内容" prop="alertContent" width="120" />

            <el-table-column align="left" label="告警状态" prop="status" width="120">
    <template #default="scope">
    {{ filterDict(scope.row.status,AlarmstatusOptions) }}
    </template>
</el-table-column>
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button v-auth="btnAuth.edit" type="primary" link icon="edit" class="table-button" @click="updateAlertsFunc(scope.row)">编辑</el-button>
    ...
    {{ detailFrom.alertLevel }}
</el-descriptions-item>
                    <el-descriptions-item label="告警类型">
    {{ detailFrom.alertType }}
</el-descriptions-item>
                    <el-descriptions-item label="告警内容">
    {{ detailFrom.alertContent }}
</el-descriptions-item>
...
const getTableData = async() => {
  const table = await getAlertsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteAlertsByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateAlertsFunc = async(row) => {
    const res = await findAlerts({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteAlertsFunc = async (row) => {
    const res = await deleteAlerts({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
