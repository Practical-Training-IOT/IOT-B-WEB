  ...
  <div>
    <div class="gva-form-box">
      <el-form :model="formData" ref="elFormRef" label-position="right" :rules="rule" label-width="80px">
        <el-form-item label="ruleId字段:" prop="ruleId">
    <el-input v-model.number="formData.ruleId" :clearable="true" placeholder="请输入ruleId字段" />
</el-form-item>
    ...
    <el-input v-model="formData.ruleDescription" :clearable="true" placeholder="请输入规则描述" />
        ...
        <el-form-item label="是否显示:" prop="isEnabled">
...
const init = async () => {
 // 建议通过url传参获取目标数据ID 调用 find方法进行查询数据操作 从而决定本页面是create还是update 以下为id作为url参数示例
    if (route.query.id) {
      const res = await findRuleInfo({ ID: route.query.id })
      if (res.code === 0) {
